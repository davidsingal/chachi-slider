...
(function($){var opts={};var counter=1;var slider={init:function(options){var target=this,timer=null,interval=null;var settings=$.extend({auto:true,pause:3000,velocity:1000,pagination:true,navigation:true,textPrev:"<",textNext:">"},options);var panelsWrapper=$("<div class=\"mod-panels-wrapper\"></div>"),wrapper=$("<div class=\"mod-wrapper\"></div>"),panels=target.find(".mod-panel");target.append(wrapper);wrapper.append(panelsWrapper);panels.appendTo(panelsWrapper);target.show();var panelWidth=target.width();function sliderSize(){panels.css({"width":panelWidth+"px","margin-right":20+"px"});panelsWrapper.css("width",((panelWidth+20)*panels.length)+"px");opts.width=panelWidth;};sliderSize();if(settings.pagination&&panels.length>1){var pagination=$("<div class=\"mod-pag\"></div>"),htmlNav="",links,t;target.append(pagination);for(var i=1;i<=panels.length;i++){htmlNav+="<li><a href=\"#panel"+i+"\">"+i+"</a></li>";}
...
opts={pagination:settings.pagination,links:links,wrapper:panelsWrapper,velocity:settings.velocity,width:panelWidth,prev:prev,next:next,panels:panels};$(window).resize(function(){panelWidth=target.width();sliderSize();});},transition:function(x){if(opts.pagination)$(opts.links).removeClass("current");opts.wrapper.stop().animate({"left":-(x-1)*(opts.width+20)},opts.velocity,function(){if(opts.pagination)$(opts.links[x]).addClass("current");(x==1)?opts.prev.hide():opts.prev.show();(x==opts.panels.length)?opts.next.hide():opts.next.show();});},startAt:function(x){counter=x;if(opts.pagination)$(opts.links).removeClass("current");opts.wrapper.css("left",-(x-1)*(opts.width+20));if(opts.pagination)$(opts.links[x]).addClass("current");(x==1)?opts.prev.hide():opts.prev.show();(x==opts.panels.length)?opts.next.hide():opts.next.show();}};$.fn.modSlider=function(method){var self=this,_arguments=arguments;return self.each(function(){if(slider[method]){return slider[method].apply(this,Array.prototype.slice.call(_arguments,1));}else if(typeof method==="object"||!method){slider.init.apply(self,_arguments);}else{$.error("No existe "+method+" en modSlider.");}});};})(jQuery);